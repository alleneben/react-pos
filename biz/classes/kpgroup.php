	...
	class kpgroup extends KPBase{
		...
		public function Add($pd){
			try {
				...
				"st"=>"Add Group",
				"sm"=>"The new group has been successfully added",
				"sd"=>$sd));	
			}
		...
		public function Edit($pd){
			try {
				...
				return json_encode(array("success"=>"true",
				"st"=>"Edit Group",
				"sm"=>"Group has been successfully updated",
				"sd"=>$sd));			
			}
			catch(ADODB_Exception $e){
				if($cnn)  $cnn->Close();
		...
		public function Delete($pd){
			try {
				...
				$cnn=$dbl->Connection();
				//create sql statement			
				$sql = "SELECT * FROM sp_group_delete($rid,$this->userid)";
				//prepare and execute sql statement (adodb)
				$stmt=$cnn->PrepareSP($sql);
				$rc=$cnn->Execute($stmt);
				if($cnn)  $cnn->Close();
				...
				return json_encode(array("success"=>"true",
				"st"=>"Delete Group",
				"sm"=>"Group has been successfully deleted"));			
			}
			catch(ADODB_Exception $e){
				if($cnn)  $cnn->Close();
				return ErrorHandler::InterpretADODB($e);
		...
		public function Search($pd){
			try {
				...
				$sd=$rc->getarray();
				if($cnn)  $cnn->Close();
				if(count($sd)<1) throw new ErrorException("No Group record found",13,1);
				$rec = array_shift($sd);
        ...
        public function Combo($pd){
			try {
				...
				$cnn=$dbl->Connection();
				//create sql statement				
				$sql = "SELECT * FROM sp_group_combo($this->userid)";
				//prepare and execute sql statement (adodb)
				$stmt=$cnn->PrepareSP($sql);
				$rc=$cnn->Execute($stmt);
				$sd=$rc->getarray();
